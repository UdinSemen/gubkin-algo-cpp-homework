...
int main() {
    srand(time(NULL));
    std::string text = "Hello! I'm doing encryption and compression.";
    std::string text1 = "AaBbCcDdEeFfGgHhIiJjKkLlMmNnOoPpQqRrSsTtUuVvWwXxYyZz";
    rsa_keys k = keys();
    std::vector <ull> res = rsa_encrypt(text, k.e, k.n);
    for (auto &a : res) {
        std::cout << a << " ";
    }
    std::cout << std::endl << rsa_decrypt(res, k.d, k.n);
    return 0;
}